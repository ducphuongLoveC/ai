...
class ProductController extends BaseController
{
    ...
    public function store()
    {
        ...
        if (sizeof($this->productModel->findProductByName($_POST['name'])) > 0) {
            $this->message['error-name'] = 'Sản phẩm này đã tồn tại';
        } else {
            $this->message['success-add'] = 'Sản phẩm đã được thêm thành công';
    ...
    public function update()
    {
        ...
        if (sizeof($this->productModel->checkNameUnique($_POST['name'], $product['name'])) == 1) {
            ...
            $this->message['success-edit'] = 'Sản phẩm đã được cập nhật thành công';
    ...
    public function delete()
    {
        ...
        if ($id && is_numeric($id) && $count[0]['count'] == 0) {
            ...
            $this->message['success-delete'] = 'Sản phẩm đã được xóa thành công';
        } else {
            $this->message['error-delete'] = "Không thể xóa sản phẩm này. Vẫn còn những sản phẩm thuộc về nó";
            // header('location: ./?module=admin&controller=product');
        }
        return $this->view('admin.product.index', [
            'data' => $products->getData(5, $page)->data,
